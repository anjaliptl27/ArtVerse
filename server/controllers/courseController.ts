...
import Course, {  ILesson, IThumbnail } from '../models/courseModel';
import User from '../models/userModel';
import Notification from '../models/notificationModel';
import mongoose from 'mongoose';
import cloudinary from '../config/cloudinaryConfig';

type UserRole = 'artist' | 'admin' | 'buyer';
...
const isValidThumbnail = (thumbnail: any): thumbnail is IThumbnail => {
  return thumbnail && 
...
const getAllCourses = async (req: Request, res: Response) => {
  try {
    ...
    if (minPrice || maxPrice) {
      filter.price = {};
      if (minPrice) filter.price.$gte = Math.round(Number(minPrice) * 100);
      if (maxPrice) filter.price.$lte = Math.round(Number(maxPrice) * 100);
    }
    ...
    switch (sort) {
      ...
      case 'popular': sortOptions.students = -1; break;
...
const addLesson = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const user = req.user;
    if (!user || user.role !== 'artist') {
    ...
    const { title, youtubeUrl, duration, resources } = req.body as ILessonRequest;
...
const publishCourse = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const user = req.user;
    if (!user || user.role !== 'artist') {
...
const approveCourse = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const user = req.user;
...
const rejectCourse = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const user = req.user;
    if (!user || user.role !== 'admin') {
...
const enrollInCourse = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const user = req.user;
